-- Comprehensive schema update for localStorage to Supabase migration
...
CREATE TRIGGER IF NOT EXISTS update_user_votes_updated_at
  BEFORE UPDATE ON user_votes
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER IF NOT EXISTS update_seller_reviews_updated_at
  BEFORE UPDATE ON seller_reviews
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER IF NOT EXISTS update_user_accounts_updated_at
...
-- 10. Create functions for vote aggregation
