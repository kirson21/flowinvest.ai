-- Fix Supabase user_bots table schema by adding missing fields that frontend expects

-- Add missing columns to user_bots table for bot creation compatibility
...
WHERE config IS NULL OR base_coin IS NULL OR quote_coin IS NULL OR trade_type IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_user_bots_is_prebuilt ON user_bots(is_prebuilt);
