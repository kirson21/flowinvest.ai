...
from supabase_client import supabase, database
...
async def get_user_bots(user_id: str, include_prebuilt: bool = True):
    ...
    try:
        ...
        if include_prebuilt:
            response = supabase.table('bots').select('*').or_(f'user_id.eq.{user_id},is_prebuilt.eq.true').order('created_at', desc=True).execute()
        else:
            response = supabase.table('bots').select('*').eq('user_id', user_id).order('created_at', desc=True).execute()
