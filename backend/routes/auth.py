...
async def get_user_subscription_limits(user_id: str):
    ...
    try:
        ...
        is_super_admin = subscription.get('plan_type') == 'super_admin'
        
        return {
            "success": True,
            "subscription": {
                "plan_type": subscription.get('plan_type', 'free'),
                "limits": subscription.get('limits') if not is_super_admin else None,
...
@router.post("/auth/user/{user_id}/subscription/check-limit")
async def check_subscription_limit(user_id: str, limit_data: dict):
    """Check if user can create a new resource based on subscription limits"""
    try:
        if not supabase_admin:
            return {"success": False, "message": "Database not available"}
        
        resource_type = limit_data.get('resource_type')  # 'ai_bots', 'manual_bots', 'marketplace_products'
        current_count = limit_data.get('current_count', 0)
        
        print(f"Checking subscription limit for user {user_id}, resource: {resource_type}, current: {current_count}")
        ...
        if not response.data or len(response.data) == 0:
            ...
            limit = default_limits.get(resource_type, 1)
...
async def get_user_subscription(user_id: str):
    ...
    try:
        ...
        return {"success": False, "message": f"Failed to get user subscription: {str(e)}"}