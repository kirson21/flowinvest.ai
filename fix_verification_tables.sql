-- Fix seller verification tables - Alternative approach
-- Run this in Supabase SQL Editor

-- Drop existing table if it exists without proper constraints
DROP TABLE IF EXISTS seller_verification_applications CASCADE;

-- Create seller_verification_applications table with proper foreign keys
CREATE TABLE seller_verification_applications (
...
-- Create indexes
CREATE INDEX idx_verification_applications_user_id ON seller_verification_applications(user_id);
CREATE INDEX idx_verification_applications_status ON seller_verification_applications(status);
...
DROP POLICY IF EXISTS "Super admin can view all verification files" ON storage.objects;
