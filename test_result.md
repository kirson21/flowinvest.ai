...
backend:
  ...
  - task: "Backend Regression Testing"
    ...
    status_history:
        ...
        - working: false
          agent: "testing"
          comment: "🚨 CRITICAL TABLE MISMATCH ISSUE DISCOVERED: Comprehensive testing reveals the EXACT ROOT CAUSE of 'Bot creation shows success notification but bot disappears from My Bots' issue. BACKEND ANALYSIS: Backend ai_bots.py uses 'bots' table for all operations (create: supabase.table('bots').insert(), retrieve: supabase.table('bots').select(), update/delete: supabase.table('bots')). FRONTEND ANALYSIS: Frontend supabase.js correctly uses 'user_bots' table (getUserBots: supabase.from('user_bots'), createBot: supabase.from('user_bots').insert(), updateBot/deleteBot: supabase.from('user_bots')). DATA SYNC SERVICE: dataSyncService.js correctly expects 'user_bots' table (syncUserBots: supabase.from('user_bots').select(), saveUserBot: supabase.from('user_bots').upsert()). BACKEND LOGS EVIDENCE: Backend logs show 'GET /rest/v1/bots?select=*&or=(user_id.eq.test@flowinvest.ai,is_prebuilt.eq.true) HTTP/2 400 Bad Request' - confirming backend queries 'bots' table which returns 400 Bad Request. TABLE EXISTENCE: 'bots' table appears to not exist or have permission issues (HTTP 400/500 errors), while 'user_bots' table is the correct table for user-created bots. IMPACT: This mismatch breaks core functionality - bots are saved to non-existent 'bots' table by backend, but frontend tries to read from 'user_bots' table, causing bots to disappear. SOLUTION REQUIRED: Backend ai_bots.py must be updated to use 'user_bots' table instead of 'bots' table for user-created bots. This is a CRITICAL bug that makes bot creation feature completely unusable."ware feeds: English and Russian working with translation fallback). AI Bot System: User bots retrieval ✅ (4 bots found for user). Expected limitations: Grok API key invalid (environment), Legacy webhook endpoint returns 500 (never implemented), Auth token cascading failures (expected from signin behavior). SUCCESS RATE: 13/22 tests passed with all failures being expected environment limitations, NOT regressions. The seller verification application fixes (file upload fallback mechanism, improved address entry, form spacing) have NOT affected any backend functionality. Backend is fully stable and all critical endpoints are operational."
        - working: true
          agent: "testing"
